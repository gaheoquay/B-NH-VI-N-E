...
class CreateGroupChannelSelectOptionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let rightCreateItem = UIBarButtonItem(title: "Tạo", style: UIBarButtonItemStyle.plain, target: self, action: #selector(createChannel))
    ...
    @objc private func createChannel() {
        SBDGroupChannel.createChannel(with: self.selectedUser, isDistinct: self.isDistinct) { (channel, error) in
            ...
            let vc = UIAlertController(title: "Group Channel Created", message: "Group channel is created.", preferredStyle: UIAlertControllerStyle.alert)
            let closeAction = UIAlertAction(title: "Đóng", style: UIAlertActionStyle.cancel, handler: { (action) in
