...
class OpenChannelListTableViewCell: UITableViewCell {
    ...
    func setModel(aChannel: SBDOpenChannel) {
        ...
        if self.channel.participantCount <= 1 {
            self.participantCountLabel.text = String(format: "%ld Participant", aChannel.participantCount)
        }
        else {
            self.participantCountLabel.text = String(format: "%ld Participants", aChannel.participantCount)
        }
    }
}
